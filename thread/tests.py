...
# Delete_thread view
    ...
    def test_delete_thread_view_without_authen(self):
        c = Client()
        responses = c.get(reverse('thread:delete_thread', args=(1, )))
        self.assertEqual(responses.status_code, 302)
    ...
    def test_delete_thread_view_not_my_thread(self):
        c = Client()
        user = User.objects.get(username='user2', email='user2@exp.com')
        c.force_login(user)
        responses = c.get(reverse('thread:delete_thread', args=(1, )))
...
# update_reply view
    ...
    def test_update_reply_view_not_my_reply(self):
        c = Client()
        user = User.objects.get(username='user2', email='user2@exp.com')
        c.force_login(user)
        responses = c.get(reverse('thread:update_reply', args=(1, )))
        self.assertEqual(responses.status_code, 302)

    def test_update_reply_view_with_post(self):
        c = Client()
        user = User.objects.get(username='user1', email='user1@exp.com')
        c.force_login(user)
        responses = c.post(reverse('thread:update_reply', args=(1, )),
                           {'content': 'content'})
        self.assertEqual(responses.status_code, 302)

    def test_update_reply_view_without_post(self):
        c = Client()
        user = User.objects.get(username='user1', email='user1@exp.com')
        c.force_login(user)
        responses = c.get(reverse('thread:update_reply', args=(1, )))
...
# Delete_reply view
    ...
    def test_delete_reply_without_auth(self):
        c = Client()
        responses = c.get(
            reverse('thread:delete_reply', args=(1,)))
        self.assertEqual(responses.status_code, 302)

    def test_delete_reply_not_my_review(self):
        c = Client()
...
# reset_report_thread
    ...
    def test_reset_report_thread_without_auth(self):
        c = Client()
        responses = c.get(
            reverse('thread:reset_report_thread', args=(1,)))
        self.assertEqual(responses.status_code, 302)

    def test_reset_report_thread_not_my_review(self):
        c = Client()
        user = User.objects.get(username='user1')
        c.force_login(user)
        responses = c.get(
...
# reset_report_reply
    ...
    def test_reset_report_reply_without_auth(self):
        c = Client()
        responses = c.get(
            reverse('thread:reset_report_reply', args=(1,)))
        self.assertEqual(responses.status_code, 302)

    def test_reset_report_reply_not_my_review(self):
        c = Client()
        user = User.objects.get(username='user1')
        c.force_login(user)
        responses = c.get(
